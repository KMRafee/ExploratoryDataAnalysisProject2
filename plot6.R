...
## Plot 6 : To compare whether Emissions from motor vehicle sources in Baltimore City
...
png(file="plot6.png", width=1040, height=640)
...
gg <- ggplot(emissionsTotal,aes(x=year,y=x),as.table=T)
      ...
      gg <- gg + scale_colour_discrete(name="City", breaks=c("24510","06037"), labels=c("Baltimore","Los Angeles"))
      gg <- gg +labs(x="Year")
      gg <- gg +labs(y="PM2.5 Emissions")
      gg <- gg +labs(title = "Emissions from motor vehicle sources - Baltimore City Vs Los Angeles")
